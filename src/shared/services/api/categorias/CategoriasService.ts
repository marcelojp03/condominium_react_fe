...
export interface ICategoriasList {
    id: number;
    name: string;
    description: string;
    status: boolean;
...
export interface ICategoriasDetail {
    id: number;
    name: string;
    email: string;
    phone: string;
...
type TCategoriasTotalCount = {
  data: ICategoriasList[];
  totalCount: number;
}

  const getAll = async (page = 1, filter = ''): Promise<TCategoriasTotalCount | any | Error> => {
    //const getAll = async (): Promise<TContactoTotalCount | Error> => {
    let categorias: ICategoriasList[];
    let total=0
    try {
