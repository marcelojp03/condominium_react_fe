...
const userSchema = z.object({
  codigo: z.string().min(1, 'Nombre es requerido'),
  zona: z.string().min(1, 'Debe seleccionar una zona'),
...
export const UnidadesDetail: React.FC = () => {
  const { id = 'nuevo' } = useParams<'id'>();
  const navigate = useNavigate();
  const [isLoading, setIsLoading] = useState(false);
  const [nombre, setNombre] = useState('');

  const [zonas, setZonas] = useState<IZonaResponse[]>([]);
  ...
  useEffect(() => {
    ZonasService.getAll().then((result) => {
...
useEffect(() => {
  if (id !== 'nuevo') {
    ...
    UnidadesService.getById(Number(id)).then((result) => {
      setIsLoading(false);
  ...
  if (id === 'nuevo') {
    UnidadesService.create(payload).then((result) => {      
    ...
    UnidadesService.updateById(Number(id), payload).then((result) => {      
      setIsLoading(false);
          ...
          <Grid size={{ xs: 12, sm: 12, md: 6 }} component="div">
            <Controller
              ...
              render={({ field }) => (
                ...
                >
                  {zonas.map((zona) => (
                    <MenuItem key={zona.idzona} value={zona.idzona.toString()}>
                      {zona.nombre}
                    </MenuItem>
                  ))}
                </TextField>
